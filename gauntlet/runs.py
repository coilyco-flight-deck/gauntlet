...
Storage layout under ``root/<run_id>/``::
    ...
    manifest.json                     # run metadata: trial_ids, started_at
    <trial_id>/iterations.jsonl      # one IterationRecord per line
    <trial_id>/holdouts.jsonl        # one HoldoutResult per line
...
class RunStore:
    ...
    def start_run(self, trial_ids: list[str]) -> str:
        ...
        for trial_id in trial_ids:
            self._trial_dir(run_id, trial_id).mkdir(parents=True, exist_ok=True)
        return run_id

    def list_trial_ids(self, run_id: str) -> list[str]:
        """Return the trials declared at ``start_run`` time."""
        ...
        ids: list[str] = list(data.get("trial_ids", []))
        return ids

    def record_iteration(self, run_id: str, trial_id: str, record: IterationRecord) -> None:
        """Append one ``IterationRecord`` to the trial's iteration buffer.
        ...
        for finding in record.findings:
            ...
            if finding.replay_bundle is None:
                _LOG.warning(
                    ...
                    "(run_id=%s trial_id=%s issue=%s). The Inspector should "
        ...
        self._append(run_id, trial_id, "iterations.jsonl", record.model_dump_json())

    def read_iteration_records(self, run_id: str, trial_id: str) -> list[IterationRecord]:
        """Return every ``IterationRecord`` previously appended for the trial.
        ...
        for line in self._read_lines(run_id, trial_id, "iterations.jsonl"):
            try:
                records.append(IterationRecord.model_validate_json(line))
            except (ValueError, TypeError) as exc:
                self._corrupt_counts[(run_id, trial_id)] = (
                    self._corrupt_counts.get((run_id, trial_id), 0) + 1
                )
                _logger.warning(
                    "Skipping corrupt IterationRecord in run_id=%s trial_id=%s: %s",
                    run_id,
                    trial_id,
                    exc,
                )
    ...
    def record_holdout_result(self, run_id: str, trial_id: str, result: HoldoutResult) -> None:
        """Append one ``HoldoutResult`` to the trial's holdout buffer."""
        if result.trial_id != trial_id:
            raise ValueError(
                f"HoldoutResult.trial_id ({result.trial_id!r}) does not match "
                f"trial_id argument ({trial_id!r})"
            )
        self._append(run_id, trial_id, "holdouts.jsonl", result.model_dump_json())

    def read_holdout_results(self, run_id: str, trial_id: str) -> list[HoldoutResult]:
        """Return every ``HoldoutResult`` previously appended for the trial.

        Corrupt lines (invalid JSON or failing schema validation) are skipped
        with a logged warning and tallied in :meth:`corrupt_record_counts`.
        """
        results: list[HoldoutResult] = []
        for line in self._read_lines(run_id, trial_id, "holdouts.jsonl"):
            try:
                ...
                _logger.warning(
                    "Skipping corrupt HoldoutResult in run_id=%s trial_id=%s: %s",
                    run_id,
                    trial_id,
                    exc,
                )
    ...
    def corrupt_record_counts(self) -> dict[tuple[str, str], int]:
        """Return a snapshot of ``(run_id, trial_id) -> corrupt-line count``.
    ...
    def _trial_dir(self, run_id: str, trial_id: str) -> Path:
        ...
        return self._run_dir(run_id) / trial_id

    def _append(self, run_id: str, trial_id: str, filename: str, payload: str) -> None:
    ...
    def _read_lines(self, run_id: str, trial_id: str, filename: str) -> list[str]:
        path = self._trial_dir(run_id, trial_id) / filename
        if not path.exists():
            return []
